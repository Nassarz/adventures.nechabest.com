...
const statIcons = [TreePine, Zap, Users, Handshake];
const statColors = ['bg-nature', 'bg-primary', 'bg-[#2D4A43]', 'bg-[#4A635D]'];

export default function Impact() {
  const { get } = useSiteContent('home');

  const stats = [
    { value: get('home.impact.stat1.value', '10,000+'), label: get('home.impact.stat1.label', 'Trees Planted'), icon: statIcons[0], color: statColors[0] },
    { value: get('home.impact.stat2.value', '1,500+'),  label: get('home.impact.stat2.label', 'Households Powered'), icon: statIcons[1], color: statColors[1] },
    { value: get('home.impact.stat3.value', '2,000+'),  label: get('home.impact.stat3.label', 'Farmers Trained'), icon: statIcons[2], color: statColors[2] },
    { value: get('home.impact.stat4.value', '15+'),     label: get('home.impact.stat4.label', 'Strategic Partners'), icon: statIcons[3], color: statColors[3] },
  ];
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center">
          ...
          >
            <div className="space-y-3 md:space-y-4">
              ...
              <h2 className="font-display text-4xl md:text-5xl font-bold text-primary leading-[1] md:leading-[0.9]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 pt-2 md:pt-4">
              {stats.map((stat, i) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: i * 0.05 + 0.3, ease: [0.22, 1, 0.36, 1] }}
          ...
          >
            <div className="relative aspect-[1/1] md:aspect-[4/5] rounded-[2.5rem] md:rounded-[4rem] overflow-hidden shadow-2xl group">
              <Image
                src={get('home.impact.image', 'https://iili.io/3oebjFS.jpg')}
            ...
            >
              <p className="text-primary font-bold text-lg md:text-xl leading-snug italic">
                &quot;{get('home.impact.quote', "Sustainability is not just a goal, it's our way of life. We are building a legacy for the next generation.")}&quot;
              </p>
              <div className="mt-6 md:mt-8 flex items-center gap-3 md:gap-4">
                ...
                <div>
                  <p className="text-[9px] md:text-[10px] font-bold text-primary uppercase tracking-[0.2em] mb-0.5 md:mb-1">
                    {get('home.impact.quoteRole', 'Project Director')}
                  </p>
                  <p className="text-xs md:text-sm text-foreground/60">
                    {get('home.impact.quoteAuthor', 'Nechabest Initiatives')}
                  </p>
                </div>
              </div>
            </motion.div>
