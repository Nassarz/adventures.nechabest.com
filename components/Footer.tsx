...
export default function Footer() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || data.message || 'Failed to subscribe.');
    ...
    <footer className="bg-[#1A3C34] text-white pt-24 pb-8 overflow-hidden relative border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          ...
          >
            <h4 className="font-display text-xl font-bold mb-8 text-white tracking-tight">Our Tours</h4>
            <ul className="space-y-4">
              {[
                'Gorilla Trekking',
                'Wildlife Safaris',
            ...
            <div className="space-y-4">
              <div className="flex gap-4">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-white/40 leading-snug font-medium">
                    Kasangati Town Council, Plot 616 Block 174, Kabanyolo, Wakiso District, Uganda
                  </p>
                </div>
              </div>
              <div className="flex gap-4">
              ...
              <div className="flex gap-4">
                ...
                <div className="text-sm">
                  ...
                  <p className="text-white/40 font-medium">{get('global.footer.phone', '+256 756 310029')}</p>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 py-12 border-t border-white/5 items-center">
          <div className="space-y-6">
            ...
            <p className="text-white/40 text-sm max-w-md font-medium">
              Subscribe to receive our latest tours, travel tips, and Uganda adventure stories.
            </p>
            <div className="flex gap-4">
              {[
          ...
          <div className="relative">
            <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-8 py-4 rounded-2xl bg-nature text-white font-bold hover:bg-white hover:text-[#1A3C34] transition-all flex items-center justify-center gap-2 shadow-lg disabled:opacity-50"
              >
                {loading ? 'Subscribing...' : 'Subscribe'} <ArrowRight className="w-4 h-4" />
              </button>
            </form>
            {success && <p className="text-nature text-xs font-bold mt-2">Thank you for subscribing!</p>}
