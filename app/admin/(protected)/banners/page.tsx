...
export default function AdminBanners() {
  ...
  return (
    <AdminLayout>
      <AdminHeader title="Banners & Campaigns" subtitle="Create campaign banners shown when visitors open the site" />
        ...
        <div className="p-6 space-y-6">
          <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => { setFormData(emptyBannerForm); setEditingId(null); setIsModalOpen(true); }}
          ...
          <div className="rounded-xl border border-nature/30 bg-nature/5 px-4 py-3 text-nature text-sm font-medium flex items-start gap-2">
            ...
            <p>Banners marked <strong>&ldquo;Show on open&rdquo;</strong> appear automatically when visitors land on the site.</p>
          </div>
                      ...
                      <tr key={banner.id} className="border-b border-black/5 hover:bg-black/2 transition-all">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <img src={banner.image} alt={banner.title} className="w-16 h-12 object-cover rounded-lg border border-black/10" referrerPolicy="no-referrer" />
                            <div>
                        ...
                        <td className="px-6 py-4">
                          <span className={`inline-flex items-center gap-1 text-xs font-bold px-3 py-1 rounded-full ${banner.showOnOpen ? 'bg-nature/15 text-nature' : 'bg-black/5 text-black/50'}`}>
                            {banner.showOnOpen ? <Eye size={13} /> : <EyeOff size={13} />}
                            {banner.showOnOpen ? 'On Open' : 'Hidden'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`text-xs font-bold px-3 py-1 rounded-full ${banner.active ? 'bg-green-500/15 text-green-700' : 'bg-black/5 text-black/50'}`}>
                            {banner.active ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex justify-center gap-2">
                            <motion.button whileHover={{ scale: 1.1 }} onClick={() => handleEdit(banner)} className="p-2 hover:bg-blue-500/20 text-blue-600 rounded-lg transition-all">
                              <Edit size={18} />
                            </motion.button>
                            <motion.button whileHover={{ scale: 1.1 }} onClick={() => handleDelete(banner.id || '')} className="p-2 hover:bg-red-500/20 text-red-600 rounded-lg transition-all">
                              <Trash2 size={18} />
                            </motion.button>
                          </div>
                        </td>
      ...
      <AnimatePresence>
        {isModalOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50" onClick={() => setIsModalOpen(false)}>
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 20 }} onClick={(e) => e.stopPropagation()} className="bg-white rounded-2xl p-6 max-w-3xl w-full max-h-[92vh] flex flex-col shadow-2xl">
              <div className="flex justify-between items-center pb-4 border-b border-black/10">
                <h2 className="text-2xl font-bold text-black">{editingId ? 'Edit Campaign Banner' : 'Add New Campaign Banner'}</h2>
                <button onClick={() => setIsModalOpen(false)} className="p-2 hover:bg-black/10 rounded-lg transition-all"><X size={24} /></button>
              </div>

              <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto py-4 space-y-4 pr-1">
                <ImageUpload label="Campaign Poster / Banner Image" value={formData.image} onChange={(url) => setFormData({ ...formData, image: url })} placeholder="Upload your campaign poster image to ImgBB" />

                <div>
                  <label className="block text-black font-bold mb-2">Banner Title *</label>
                  <input type="text" value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} placeholder="e.g., Gorilla Trekking Season 2026" className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" required />
                </div>

                <div>
                  <label className="block text-black font-bold mb-2">Subtitle</label>
                  <input type="text" value={formData.subtitle} onChange={(e) => setFormData({ ...formData, subtitle: e.target.value })} placeholder="Short catchy line" className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" />
                </div>

                <div>
                  <label className="block text-black font-bold mb-2">Description</label>
                  <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} placeholder="A couple of sentences about the campaign..." rows={3} className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" />
                </div>
                ...
                <div className="rounded-lg border border-black/10 p-4 space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input type="text" value={formData.ctaLabel} onChange={(e) => setFormData({ ...formData, ctaLabel: e.target.value })} placeholder="Book Now" className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" />
                    </div>
                    <div>
                      <label className="block text-black font-bold mb-2">Button Action</label>
                      <select value={formData.ctaType} onChange={(e) => setFormData({ ...formData, ctaType: e.target.value as 'link' | 'whatsapp' })} className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature bg-white">
                  ...
                  {formData.ctaType === 'whatsapp' ? (
                    ...
                    <div>
                      ...
                      <input type="url" value={formData.ctaLink} onChange={(e) => setFormData({ ...formData, ctaLink: e.target.value })} placeholder="https://adventures.nechabest.com/adventures" className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" />
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="rounded-lg border border-black/10 p-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={formData.showOnOpen} onChange={(e) => setFormData({ ...formData, showOnOpen: e.target.checked })} className="h-4 w-4" />
                      <div>
                  ...
                  <div className="rounded-lg border border-black/10 p-4">
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={formData.active} onChange={(e) => setFormData({ ...formData, active: e.target.checked })} className="h-4 w-4" />
                      <div>
                ...
                <div>
                  ...
                  <input type="number" min={0} value={formData.order} onChange={(e) => setFormData({ ...formData, order: parseInt(e.target.value) || 0 })} className="w-full px-4 py-2 border border-black/10 rounded-lg focus:outline-none focus:border-nature" />
                </div>

                <div className="flex gap-4 pt-6 border-t border-black/10">
                  <motion.button type="submit" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} disabled={saving} className="flex-1 bg-nature text-black font-bold py-3 rounded-lg hover:shadow-lg transition-all disabled:opacity-50">
                    {saving ? 'Saving...' : editingId ? 'Update Banner' : 'Create Banner'}
                  </motion.button>
                  <motion.button type="button" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setIsModalOpen(false)} className="flex-1 bg-black/10 text-black font-bold py-3 rounded-lg hover:bg-black/20 transition-all">
                    Cancel
                  </motion.button>
                </div>
              </form>
